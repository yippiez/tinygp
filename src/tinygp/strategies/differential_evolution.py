...
class DifferentialEvolution:
    ...
    def tell(self, state: StrategyState, fitness: np.ndarray) -> StrategyState:
        ...
        pool = [state.population[int(i)] for i in order[: max(self.to_k, 4)]]
        next_population = [pool[0]]
        while len(next_population) < self.population_size:
            a, b, c = self._rng.sample(pool, 3)
            donor = uop_crossover(a, b, self._rng)
            donor = uop_crossover(donor, c, self._rng)
            trial = donor
            if self._rng.random() < self.mutation_rate:
                trial = uop_mutate(trial, self._rng, self.max_depth, lambda depth: uop_random_tree(depth, self._rng, self._var_x, self._unary_ops, self._binary_ops, self.const_min, self.const_max))
