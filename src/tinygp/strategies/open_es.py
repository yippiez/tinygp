...
class Open_ES:
    ...
    def tell(self, state: StrategyState, fitness: np.ndarray) -> StrategyState:
        ...
        utilities = (self.population_size - 1 - ranks) / max(1.0, self.population_size - 1)
        utilities = utilities - np.mean(utilities)
        weights = (np.abs(utilities) + 1e-6).tolist()
        parents = [state.population[i] for i in self._rng.choices(range(self.population_size), weights=weights, k=max(self.to_k, 4))]

        next_population = []
        while len(next_population) < self.population_size:
            anchor = self._rng.choice(parents)
            twin = uop_mutate(anchor, self._rng, self.max_depth, lambda depth: uop_random_tree(depth, self._rng, self._var_x, self._unary_ops, self._binary_ops, self.const_min, self.const_max))
            a = anchor
            b = twin
            if self._rng.random() < self.crossover_rate:
                a = uop_crossover(anchor, twin, self._rng)
                b = uop_crossover(twin, anchor, self._rng)
            next_population.append(a)
            if len(next_population) < self.population_size:
                next_population.append(b)
