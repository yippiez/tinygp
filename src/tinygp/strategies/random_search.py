...
class RandomSearch:
    ...
    def tell(self, state: StrategyState, fitness: np.ndarray) -> StrategyState:
        ...
        next_population = [uop_random_tree(self.max_depth, self._rng, self._var_x, self._unary_ops, self._binary_ops, self.const_min, self.const_max) for _ in range(self.population_size)]
