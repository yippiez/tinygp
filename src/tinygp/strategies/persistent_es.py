...
class PersistentES(BasicStrategy):
    """PersistentES evolutionary strategy for GP populations.
    ...
    def tell(self, state: BasicStrategyState, fitness: np.ndarray) -> BasicStrategyState:
        ...
        if not self._carry:
            self._carry = tuple(elites)
        carry = list(self._carry[: max(1, self.to_k // 2)])
        parent_pool = carry + elites
        next_population = list(parent_pool[: max(1, self.to_k // 2)])
        ...
        self._carry = tuple(parent_pool[: self.to_k])
