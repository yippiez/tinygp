...
class NoiseReuseES:
    ...
    def tell(self, state: StrategyState, fitness: np.ndarray) -> StrategyState:
        ...
        if not self._noise_bank:
            self._noise_bank = [uop_mutate(elites[0], self._rng, self.max_depth, lambda depth: uop_random_tree(depth, self._rng, self._var_x, self._unary_ops, self._binary_ops, self.const_min, self.const_max)) for _ in range(max(2, self.to_k // 2))]
        ...
        while len(next_population) < self.population_size:
            ...
            if len(parent_pool) > 1 and self._rng.random() < self.crossover_rate:
                child = uop_crossover(child, self._rng.choice(parent_pool), self._rng)
        ...
        self._noise_bank = [uop_mutate(self._rng.choice(elites), self._rng, self.max_depth, lambda depth: uop_random_tree(depth, self._rng, self._var_x, self._unary_ops, self._binary_ops, self.const_min, self.const_max)) for _ in range(max(2, self.to_k // 2))]
