...
class SV_OpenES:
    ...
    def tell(self, state: StrategyState, fitness: np.ndarray) -> StrategyState:
        ...
        centered = (self.population_size - 1 - ranks) / max(1.0, self.population_size - 1)
        centered = centered - np.mean(centered)
        weights = (np.abs(centered) + 1e-6).tolist()
        ...
        while len(next_population) < self.population_size:
            ...
            if len(parents) > 1 and self._rng.random() < self.crossover_rate:
                child = uop_crossover(child, self._rng.choice(parents), self._rng)
