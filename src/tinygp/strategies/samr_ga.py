...
class SAMR_GA:
    ...
    def tell(self, state: StrategyState, fitness: np.ndarray) -> StrategyState:
        ...
        self._self_mutation = float(np.clip(self._self_mutation * np.exp(self._rng.gauss(0.0, 0.15)), 0.01, 1.0))
        ...
        while len(next_population) < self.population_size:
            ...
            child = uop_crossover(parent_a, parent_b, self._rng) if self._rng.random() < self.crossover_rate else parent_a
