...
class LearnedGA:
    ...
    def tell(self, state: StrategyState, fitness: np.ndarray) -> StrategyState:
        ...
        if not self.maximize:
            adv = -adv
        self._self_mutation = float(np.clip(self._self_mutation + 0.01 * adv + self._rng.uniform(-0.02, 0.02), 0.01, 1.0))
        self._self_crossover = float(np.clip(self._self_crossover + self._rng.uniform(-0.03, 0.03), 0.05, 0.95))
        ...
        while len(next_population) < self.population_size:
            ...
            child = uop_crossover(parent_a, parent_b, self._rng) if self._rng.random() < self._self_crossover else parent_a
