...
class Sep_CMA_ES:
    ...
    def __init__(
        ...
        *,
        ...
        self._sigma = 1.0
        self._axis_scale = np.ones(8, dtype=float)
    ...
    def tell(self, state: StrategyState, fitness: np.ndarray) -> StrategyState:
        ...
        self._sigma = float(np.clip(self._sigma * np.exp(0.1 * (spread - 1.0)), 0.05, 4.0))
        self._axis_scale = np.clip(0.9 * self._axis_scale + 0.1 * self._sigma, 0.05, 4.0)
        ...
        while len(next_population) < self.population_size:
            ...
            if len(elites) > 1 and self._rng.random() < self.crossover_rate:
                child = uop_crossover(child, self._rng.choice(elites), self._rng)
