...
class PGPE:
    ...
    def tell(self, state: StrategyState, fitness: np.ndarray) -> StrategyState:
        ...
        self._center = elites[0]
        next_population = [self._center]
        while len(next_population) < self.population_size:
            child = uop_mutate(self._center, self._rng, self.max_depth, lambda depth: uop_random_tree(depth, self._rng, self._var_x, self._unary_ops, self._binary_ops, self.const_min, self.const_max))
            next_population.append(child)
            if len(next_population) < self.population_size:
                mirror = uop_mutate(self._center, self._rng, self.max_depth, lambda depth: uop_random_tree(depth, self._rng, self._var_x, self._unary_ops, self._binary_ops, self.const_min, self.const_max))
                next_population.append(mirror)
